 ...
 * @author Francisco
 */
public class ProjectMenuController1 implements Initializable {
    ...
    private static final Logger logger = Logger.getLogger(ProjectMenuController.class.getName());
    ...
    public void analysis(MouseEvent event, String id) {
        ...
        Integer indexDockable = getIndexNewDockable(countAux, controlA);

        JPanel panelA = null;
        JPanel panelAux = null;

        if (indexDockable != null && indexDockable >= 0 && indexDockable < controlA.getCDockableCount()) {
            DockableElement docka = (DockableElement) controlA.getCDockable(indexDockable);
            panelA = (JPanel) docka.getContentPane();

            if (indexDockable - 1 >= 0 && indexDockable - 1 < controlA.getCDockableCount()) {
                DockableElement docka2 = (DockableElement) controlA.getCDockable(indexDockable - 1);
                panelAux = (JPanel) docka2.getContentPane();
            } else {
                logger.warning("Dockable index (aux) is out of bounds: " + (indexDockable - 1));
            }
        } else {
            logger.warning("Dockable index is out of bounds or null. Current index: " + indexDockable + ", count: " + countA);
            return;
        }
        ...
        StackPane pane2 = new StackPane(nodo);
        StackPane pane3 = new StackPane(nodo2);
        ...
        if (gridPane.getRowConstraints().isEmpty()) {
            for (int i = 0; i < 3; i++) {
